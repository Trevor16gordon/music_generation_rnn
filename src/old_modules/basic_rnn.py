...
def model_note_time(learning_rate=0.001, total_vicinity=53, dropout=0, recurrent_dropout=0):
    ...
    mod_notes = tf.keras.layers.LSTM(50, return_sequences=True, dropout=dropout, stateful=False)(mod_notes)
    ...
    class haltCallback(tf.keras.callbacks.Callback):
        def on_batch_end(self, batch, logs={}):
            if(logs.get('loss') <= 0.005):
                print("\n\n\nReached 0.005 loss value so cancelling training!\n\n\n")
    ...
    for l in range(size):
        ...
        for i in range(num_notes):
            ...
            if i == 0:
                ...
                old_input_b = model.layers[5]
                concat_lay = model.layers[6]([new_input, old_input_b.output])
                mod_notes = model.layers[7](concat_lay)
                mod_notes = model.layers[8](mod_notes)
                mod_notes = model.layers[9](mod_notes)
...
class RNNMusicExperimentTen(RNNMusicExperiment):
    ...
    def run(self):
        ...
        print("Training...")
        self.model, self.history = self.train_model(self.prepared_data)
        self.model.save('/Users/trevorgordon/Library/Mobile Documents/com~apple~CloudDocs/Documents/root/Columbia/Spring2022/NNDL/music_generation_rnn/models/latest')
        self.predict_and_save_data()
...
if __name__ == "__main__":
    ...
    print("Trying Exp 11")
    exp = RNNMusicExperimentEleven(learning_rate=0.0001, num_music_files=10, dropout=0)
    exp.run()
